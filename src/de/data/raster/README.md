...
1. Legen Sie einen neuen Datenspeicher `sfdem` an, über den ein GeoTIFF
...
1. Legen Sie einen neuen Datenspeicher `mosaic_sample` an, über den ein ImageMosaic
...
1. Legen Sie einen neuen Datenspeicher `wms_nrw` an, über den ein bereits existierender WMS
...
2. Veröffentlichen Sie einen Dienst Ihrer Wahl (z.B. `WMS_NW_DGM-SCHUMMERUNG`) über den GeoServer!
